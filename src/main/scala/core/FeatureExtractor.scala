...
object FeatureExtractor {
  def apply(sample: Sample): Option[Seq[Double]] = {
    //TODO implement
    if (prune(sample.matrix)) None
    else {
      val subMxs = getSubMatrixs(getScaledMatrix(threshold(sample.matrix)))
      Some(littleProjectionLength(subMxs) ++ subMxs.map(_.map(_.sum).sum))
    }
  }
  ...
  private def prune(matrix: Array[Array[Double]]) = {
    matrix.map(_.find(_ > 0.3)).find(_.nonEmpty).isEmpty
  }
